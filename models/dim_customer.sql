...
{{ config(
    materialized='incremental',
    unique_key='customer_account_number',
    pre_hook="UPDATE DIM_CUSTOMER SET Active_Flag = 0, Modified_At = CURRENT_TIMESTAMP() WHERE Active_Flag = 1"
) }}
...
ivr AS (
    SELECT
        MD5(LEFT("ACCOUNT",100) || email) AS customer_id,  -- surrogate key since CUSTOMER ID is missing
        NULL AS brand_id,
        LEFT("ACCOUNT",100) AS customer_account_number,
        email AS customer_email,
        '' AS customer_segment,
...
subease AS (
    SELECT
        MD5(customer_email || customer_first_name || customer_last_name) AS customer_id,  -- surrogate key
        NULL AS brand_id,
        '' AS customer_account_number,
        customer_email,
        '' AS customer_segment,
